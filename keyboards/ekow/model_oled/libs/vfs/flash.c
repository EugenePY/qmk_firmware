...
int flashSectorErase(flashsector_t sector) {
    ...
    if (flashUnlock() == CH_FAILED) return FLASH_RETURN_NO_PERMISSION;
    ...
    if (flashIsErased(flashSectorBegin(sector), flashSectorSize(sector)) == FALSE) return FLASH_RETURN_BAD_FLASH; /* Sector is not empty despite the erase cycle! */
...
bool flashIsErased(flashaddr_t address, size_t size) {
    ...
    while (size > 0) {
        if (*(char*)address != 0xff) return FALSE;
        ++address;
        --size;
    }
...
bool flashCompare(flashaddr_t address, const char* buffer, size_t size) {
     ...
     * then, fallback to byte per byte comparison. */
    while (size >= sizeof(flashdata_t)) {
        if (*(volatile flashdata_t*)address != *(flashdata_t*)buffer) return FALSE;
        address += sizeof(flashdata_t);
        buffer += sizeof(flashdata_t);
        size -= sizeof(flashdata_t);
    }
    while (size > 0) {
        if (*(volatile char*)address != *buffer) return FALSE;
        ++address;
        ++buffer;
        --size;
    }
...
int flashWrite(flashaddr_t address, const char* buffer, size_t size) {
    ...
    if (alignOffset != 0) {
        ...
        if (chunkSize > size) chunkSize = size; // this happens when both address and address + size are not aligned
